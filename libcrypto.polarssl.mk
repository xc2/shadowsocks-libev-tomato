...
POLARSSL_INSTALL = $(CACHEROOT)/polarssl-$(POLARSSL_VERSION)
...
$(POLARSSL_INSTALL): $(POLARSSL_TARBALL)
	mkdir -p "$(POLARSSL_SOURCE_DIR)"
	tar zxf "$(POLARSSL_TARBALL)" -C "$(POLARSSL_SOURCE_DIR)" --strip-components 1
	(cd "$(POLARSSL_SOURCE_DIR)" && make $(MKENV) lib && make DESTDIR=$(POLARSSL_INSTALL) install)
